    ...
    public function action_index() {
        ...
        if (is_post()) {
            if ($this->form->validate()) {
                ...
                $formfile = 'listform-'.$name.'.php';
                
                $data = $this->form->asArray();
                file_put_contents($f.'/config/codegen/'.$formfile, "<?php\n\nreturn ".var_export($data, true) . ";\n\n");
                
                $generator = new codegen\generator\ListFormGenerator();
                if ($generator->loadData( $module_name, $formfile )) {
                    $generator->generate();
                }
                
                redirect( '/?m=codegen&c=listformgenerator&fm='.urlencode($module_name).'&ff='.urlencode($formfile) );
            }
        }
